...
class CDetailKRSMerdeka extends MainPageM {
	/**
	* total SKS
	*/
	static $totalSKS=0;
	...
	*/
    ...
    public function itemBound ($sender,$param) {
        ...
        if ($item->ItemType === 'Item' || $item->ItemType === 'AlternatingItem') {
            if ($item->DataItem['batal']) {
                ...
                CDetailKRSMerdeka::$totalSKSBatal+=$item->DataItem['sks'];
                CDetailKRSMerdeka::$jumlahMatkulBatal+=1;
            }else{
                $idkrsmatkul=$item->DataItem['idkrsmatkul'];
                $idpenyelenggaraan=$item->DataItem['idpenyelenggaraan'];
                $idkelas=$_SESSION['currentPageKRSMerdeka']['DataMHS']['kelas_dulang'];
                ...
                CDetailKRSMerdeka::$totalSKS+=$item->DataItem['sks'];
                CDetailKRSMerdeka::$jumlahMatkul+=1;
            }
        }
    }
    ...
    public function prosesKelas ($sender,$param) {
        ...
        if ($idkelas_mhs=='none') {
            ...
            $this->redirect('perkuliahan.DetailKRS', true,array('id'=>$_SESSION['currentPageKRSMerdeka']['DataKRS']['krs']['idkrs']));
        }else {
            $jumlah_peserta_kelas = $this->DB->getCountRowsOfTable ("kelas_mhs_detail WHERE idkelas_mhs=$idkelas_mhs",'idkelas_mhs');
    ...
    public function tambahKRS ($sender,$param) {
        ...
        $datakrs=$_SESSION['currentPageKRSMerdeka']['DataKRS'];
        $idsmt=$datakrs['krs']['idsmt'];
        $tahun=$datakrs['krs']['tahun'];        
        $this->Nilai->setDataMHS($_SESSION['currentPageKRSMerdeka']['DataMHS']);
        if ($idsmt==3) {
            $this->createObj('Finance');
            $this->Finance->setDataMHS($_SESSION['currentPageKRSMerdeka']['DataMHS']);
    ...
    public function closeDetailKRS ($sender,$param) { 
        unset($_SESSION['currentPageKRSMerdeka']);
	...
	public function printKRS ($sender,$param) {
        ...
        switch ($_SESSION['outputreport']) {
            ...
            case  'pdf' :                
                ...
                $dataReport=$_SESSION['currentPageKRSMerdeka']['DataMHS'];
